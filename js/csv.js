  ...
  for (var i = 0; i < wTR.length; i++) {
    ...
    for (var j = 0; j < wTD.length; j++) {
      if (wTD[j].getAttribute("cmanSortBtn") !== null) {
        ...
        wAddBtn +=
          '<svg class="tsImg" id="ts_A_' +
          j +
          '" onclick="tSort(this)"><path d="M4 0 L0 6 L8 6 Z"></path></svg>';
        wAddBtn +=
          '<svg class="tsImg" id="ts_D_' +
          j +
          '" onclick="tSort(this)"><path d="M0 0 L8 0 L4 7 Z"></path></svg>';
        wAddBtn += "</div>";
...
function tSort(argObj) {
  ...
  for (var i = 0; i < wMoveRow.length; i++) {
    ...
    // 最終行にコピーしてから移動元を削除
    wLastTr.parentNode.insertBefore(
      wMoveTr.cloneNode(true),
      wLastTr.nextSibling
    );
