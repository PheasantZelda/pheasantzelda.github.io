...
document.addEventListener('click', function (e) {
  if (e.target.classList.contains('add-mu-box')) {
    ...
    box.after(clone);
    // 追加したMU_boxにもドラッグイベントを付与
    setMUBoxDraggable([clone]);
  }
  // 行削除
  if (e.target.classList.contains('remove-mu-box')) {
    const box = e.target.closest('.MU_box');
    if (document.querySelectorAll('.MU_box').length > 1) {
...
// 設定ボタンで色変更UIをポップアップ表示
document.addEventListener('click', function (e) {
  if (e.target.classList.contains('mu-box-settings')) {
    // 既存のポップアップがあれば消す
    document.querySelectorAll('.mu-settings-popup').forEach((p) => p.remove());
    ...
    popup.querySelector('.bg_color1').oninput = function () {
      title.style.background = `linear-gradient(90deg, ${this.value}, ${
        popup.querySelector('.bg_color2').value
      })`;
    };
    popup.querySelector('.bg_color2').oninput = function () {
      title.style.background = `linear-gradient(90deg, ${
        popup.querySelector('.bg_color1').value
      }, ${this.value})`;
    };
  } else {
    // ポップアップ外クリックで閉じる
